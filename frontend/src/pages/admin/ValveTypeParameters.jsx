...
import "./ValveTypeParameters.css";

function ValveTypeParameters() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch valve type details
      const valveTypeRes = await axios.get(`/valve-types/${id}`);
      setValveType(valveTypeRes.data.data);
      
      // Fetch linked parameters
      const linkedRes = await axios.get(`/valve-types/${id}/parameters`);
      setLinkedParameters(linkedRes.data.data || []);
      
      // Fetch available parameters
      const availableRes = await axios.get(`/valve-types/${id}/available-parameters`);
  ...
  const handleLinkParameter = async (e) => {
    ...
    try {
      await axios.post(`/valve-types/${id}/parameters`, {
        parameterId: parseInt(selectedParameter)
      });
  ...
  const handleUnlinkParameter = async (parameterId) => {
    if (!window.confirm("Are you sure you want to remove this parameter from this valve type?")) {
      return;
    }

    try {
      await axios.delete(`/valve-types/${id}/parameters/${parameterId}`);
  ...
  if (loading) {
    return (
      <div className="valve-params-container">
  ...
  return (
    <div className="valve-params-container">
      <div className="valve-params-header">
        <div>
          <button className="btn-back" onClick={() => navigate("/admin/valve-config/valve-types")}>
            ← Back to Valve Types
          </button>
          <h1 className="valve-params-title">
            {valveType?.name} - Parameters
          </h1>
          <p className="valve-params-subtitle">
            Manage parameters for this valve type
          </p>
        </div>
        <button
      ...
      <div className="params-section">
        ...
        {linkedParameters.length === 0 ? (
          <div className="empty-state">
            <p>No parameters linked to this valve type yet.</p>
...
export default ValveTypeParameters;
